...
#define LG_ButtonColor_Selected [UIColor colorWithRed:0.0/255 green:150.0/255 blue:230.0/255 alpha:100]
#define LG_ButtonColor_UnSelected [UIColor colorWithRed:51.0/255 green:51.0/255 blue:51.0/255 alpha:100]
#define LG_BannerColor [UIColor colorWithRed:0.0/255 green:150.0/255 blue:230.0/255 alpha:100]
...
-(void)commonInit {
    //按钮名称
    NSMutableArray *titleList = [[NSMutableArray alloc]initWithObjects:@"详情",@"检测",@"评论", nil];
...
- (void)createItem:(NSMutableArray *)item {
    ...
    CGFloat marginX = (self.frame.size.width)/(count);
    for (int i = 0; i<count; i++) {
        
        NSString *temp = [self.titleList objectAtIndex:i];
        //按钮的X坐标计算，i为列数
        CGFloat buttonX = i * ( marginX);
        UIButton *buttonItem = [[UIButton alloc] initWithFrame:CGRectMake(buttonX, 0, Main_Screen_Width/self.titleList.count, self.frame.size.height)];
        //设置
        buttonItem.tag = i + 1;
...
-(void)creatBanner:(CGFloat)firstX{
    ...
    LGLayer.frame = CGRectMake(firstX, self.frame.size.height - 2, Main_Screen_Width/self.titleList.count, 2);
    // 设定它的frame
